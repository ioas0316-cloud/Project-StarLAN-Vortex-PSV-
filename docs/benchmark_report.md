...
|---|---|---|
| **총 소요 시간 (ns)** | 10,160,188,136 ns | 607,724,529 ns |
| **초당 처리량 (OPS)** | 4,921.17 OPS | 82,274.12 OPS |
...
- 64-bit `double` 정밀도 텐서 연산이 파이썬의 GIL 병목을 완벽히 소멸시키고, 정적 VRAM 풀(Static VRAM Pool) 기반 계산으로 무거운 드라이버 쿼리 지연을 회피함으로써 94.02%에 달하는 압도적 처리량 격차를 증명해 냈습니다.
